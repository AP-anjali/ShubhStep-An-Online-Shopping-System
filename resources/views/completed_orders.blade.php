                                ...
                                @foreach($orders_data as $data)
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            @if($data->is_seven_days_complte_to_delivery == 0)
                                                <form method="POST" action="{{ route('return_request_reason_form') }}">
                                                    @csrf
                                                    <input type="hidden" name="order_id" value = "{{ $data->id }}">
                                                    <button type="submit" style="border: none;" class="badge badge-primary badge-shadow">Return</button>
                                                </form>
                                            @endif

                                            @if($data->is_seven_days_complte_to_delivery == 1)
                                                <button type="button" style="border: none; opacity : 0.7; pointer-events : none;" class="badge badge-primary badge-shadow">Return</button>
                                            @endif

                                            @if($data->is_seven_days_complte_to_delivery == 0)
                                                <form method="POST" action="{{ route('exchange_request_reason_form') }}">
                                                    @csrf
                                                    <input type="hidden" name="order_id" value = "{{ $data->id }}">
                                                    <button type="submit" style="border: none; margin-top : 4px;" class="badge badge-primary badge-shadow">Exchange</button>
                                                </form>
                                            @endif

                                            @if($data->is_seven_days_complte_to_delivery == 1)
                                                <button type="button" style="border: none; margin-top : 4px; opacity : 0.7; pointer-events : none;" class="badge badge-primary badge-shadow">Exchange</button>
                                            @endif

                                            @if($data->feedback_date == NULL)
                                                <form method="POST" action="{{ route('order_feedback_form') }}" style = " margin-top : 4px;">
                                                    @csrf

                                                    <input type="hidden" name="order_id" value = "{{ $data->id }}">
                                                    <input type="hidden" name="customer_id" value = "{{ $customer_session->id }}">
                                                    <button type="submit" style="border: none;" class="badge badge-primary badge-shadow">Feedback</button>
                                                </form>
                                            @else
                                                <button style="border: none; opacity : 0.7; pointer-events : none; margin-top : 4px;" class="badge badge-primary badge-shadow">Feedback</button>
                                            @endif
