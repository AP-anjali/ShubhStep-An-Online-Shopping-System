<main>
    ...
    <div class="product-container">
      ...
      <div class="container">
        ...
        <div class="product-box" style="width: 100%;">
            ...
            <h2 class="title" style="text-align : center; font-size : 1.2rem; color : #8789ff; padding-top: 0.5rem;">WHOLESALE SHOPPING</h2>

            @php
                $grandTotalWithoutDiscount = 0;
            ...
            @foreach($cart_data as $cart_product)
                ...
                <div class="product-featured" style="margin-top: 1%;" id="product-{{ $index }}">
                    <h2 class="title" style="border-top: 1px solid hsl(0, 0%, 93%); padding-top: 0.5rem;">{{ $product->product_title }}</h2>
                        ...
                        <div class="showcase-container">
                            <div class="showcase">
                                ...
                                <div class="showcase-banner" style="text-align: center; width : 100%; display: flex; align-items: center; justify-content: center;">
                                  <img id="main-image-{{ $index }}" src="{{ asset('storage/' . $product->thumbnail_image ) }}" style = "object-fit: contain; min-height: 320px; max-height: 400px;" width = "100%" alt="product img" class="showcase-img">
                                </div>

                                <div class="showcase-content">
                                  
                                    <div style="height: 5rem; display: flex; margin-bottom: 0.7rem; text-align: center; align-items: center; justify-content: space-between;">
                                        <img src="{{ asset('storage/' . $product->thumbnail_image) }}" style="height: 100%; border: 2px solid #8789ff; border-radius: 0.4rem; cursor: pointer;" class="showcase-img thumbnail-img">

                                        @if(is_array($product->other_images) && count($product->other_images) > 0)
                                            @foreach($product->other_images as $img)
                                                <img src="{{ asset('storage/' . $img) }}" style="height: 100%; border: 2px solid #454545; border-radius: 0.4rem; cursor: pointer;" class="showcase-img thumbnail-img">
                                            @endforeach
                                        @endif
                                    </div>
                                    ...
                                    <a onclick="window.location.reload();" style="cursor: pointer;">
                                        <h3 class="showcase-title">{{ $product->product_title }}</h3>
                                    </a>
                                    ...
                                    <div class="price-box">
                                        <p class="price"><i class="fa-solid fa-indian-rupee-sign"></i> {{ $product->price_with_discount }}/-</p>
                                        <del>{{ $product->price_without_discount }}</del>
                                    </div>

                                    <h3 class="showcase-title" style="margin-top: 5%; color: #454545;">Ordered quantity : {{ $cart_product['quantity'] }}</h3>

                                    <h3 class="showcase-title" style="margin-top: 3%; color: #8789ff;">Total Amount : </h3>

                                    <div style="display: flex; gap: 5%; text-align: center; align-items: center; justify-content: center; margin-top: 1%;">

                                        <h3 class="showcase-desc" style="color: #454545; font-size : 0.9rem; font-weight : 650; text-transform: uppercase;">Without discount : <span style="color: #E10600;"><i class="fa-solid fa-indian-rupee-sign"></i> {{ $product->price_without_discount * $cart_product['quantity'] }}/-</span></h3>
                                        <h3 class="showcase-desc" style="color: #454545; font-size : 0.9rem; font-weight : 650; text-transform: uppercase;">With discount : <span style="color: #006400;"><i class="fa-solid fa-indian-rupee-sign"></i> {{ $product->price_with_discount * $cart_product['quantity'] }}/-</span></h3>

                                    </div>

                                        @php
                                            $grandTotalWithoutDiscount += $product->price_without_discount * $cart_product['quantity'];
                                            $grandTotalWithDiscount += $product->price_with_discount * $cart_product['quantity'];
                                        @endphp
                        ...
                        <div class="showcase-container">
                            <div class="showcase">
                                ...
                                <div class="showcase-content">          
                                    ...
                                    <h3 class="showcase-title" style="margin-top: 2%;text-align : center; color: #8789ff; font-size : 1.1rem;">Grand Total : </h3>

                                    <div style="display: flex; gap: 5%; text-align: center; align-items: center; justify-content: center; margin-top: 1%; word-wrap: break-word; ">
                                        <h3 class="showcase-desc" style="color: #454545; font-size : 1rem; font-weight : 650; text-transform: uppercase;">Without discount : <span style="color: #E10600;"><i class="fa-solid fa-indian-rupee-sign"></i> {{ $grandTotalWithoutDiscount }}/-</span></h3>
                                        <h3 class="showcase-desc" style="color: #454545; font-size : 1rem; font-weight : 650; text-transform: uppercase;">With discount : <span style="color: #006400;"><i class="fa-solid fa-indian-rupee-sign"></i> {{ $grandTotalWithDiscount }}/-</span></h3>
                                    </div>

                                    <div style="display: flex; gap: 5%; text-align: center; align-items: center; justify-content: center; margin-top: 3%;">
                                        <button  class="add-cart-btn" id="rzp-button">Online Payment</button>
                                        <button class="add-cart-btn" id="COD-button" onclick = "addModal()">Cash on delivery</button>
                                        <button class="add-cart-btn" onclick="window.location.href = '/'">Exit</button>
                                    </div>
                                      
                                      <form method="post" action="{{ route('payment_of_wholesale_product') }}" style = "display : none;" id="payment-form">
                                          @csrf   
                                      ...
                                      <form method="post" action="{{ route('payment_of_wholesale_product_COD') }}" style = "display : none;" id = "payment-form-COD">
                                          @csrf

                                            <input type="hidden" name="totalAmountWithDiscount" value = "{{ $totalAmountWithDiscount }}">
  ...
  <script src="https://checkout.razorpay.com/v1/checkout.js"></script>
    ...
    <script>
        ...
        var totalAmountWithDiscount = {{ $grandTotalWithDiscount }};
        ...
        document.getElementById('rzp-button').onclick = function (e) {
            ...
            @foreach($cart_data as $cart_product)
                ...
                paymentForm.appendChild(productIdInput);
                ...
                productQuantityInput.value = '{{ $cart_product['quantity'] }}';
                paymentForm.appendChild(productQuantityInput);
            @endforeach
        ...
        function tryfunction(e) 
        {
            ...
            e.preventDefault();
            var paymentForm2 = document.getElementById('payment-form-COD');
            ...
            @foreach($cart_data as $cart_product)
                ...
                productIdInput.name = 'product_id[]';
                productIdInput.value = '{{ $product->id }}';
                paymentForm2.appendChild(productIdInput);

                var productQuantityInput = document.createElement('input');
                productQuantityInput.type = 'hidden';
                productQuantityInput.name = 'product_quantity[]';
                productQuantityInput.value = '{{ $cart_product['quantity'] }}';
                paymentForm2.appendChild(productQuantityInput);
            @endforeach
        ...
        function addModal(e)
        {
          ...
          swal({
                title: 'Are you sure?',
                text: 'Are you sure to buy these all products by "cash-on-delivery" payment method !',
                icon: 'warning',
                buttons: true,
                dangerMode: true,
        ...
        document.getElementById('COD-button').onclick = addModal;
    </script>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            const productContainers = document.querySelectorAll(".product-featured");

            productContainers.forEach((productContainer, index) => {
                const thumbnails = productContainer.querySelectorAll(".thumbnail-img");
                const mainImage = productContainer.querySelector(`#main-image-${index}`);
